...
public class ExcelControl implements ExcelInterface {
            ...
            if (check) {
                ...
                if (!cells.get(16).isEmpty()) {
                    if (cells.get(16).contains(",")) {
                        ...
                        Number number;
                        try {
                            number = format.parse(cells.get(16).substring(1, cells.get(16).length()-1));
                            float d = number.floatValue();
                            kw.setRatioCompetition(d);
                        } catch (ParseException ex) {
                            Logger.getLogger(ExcelControl.class.getName()).log(Level.SEVERE, null, ex);
                        }

                        //System.out.println(cells.get(16));
                        //kw.setRatioCompetition(Float.parseFloat(d);
                    } else {
    ...
    public void executeSwing(Object data, JLabel lblResult, JTextArea textResultKeywords, int type) {
        textResultKeywords.setText("");
        System.out.println(data.toString());
        List<String> cells = null;
        List<Keyword> lstKeyWord = new ArrayList<Keyword>();
        ...
        int settings = ValueFormat.COMMAS;
        lblResult.setText(ValueFormat.format(Long.valueOf(total * 30), settings));
    }
